...
def map():
    ...
    return render_template('map.html', devices = api_bp.devices().json, masterlist = api_bp.usageoffline())

@dashboard_bp.route("/benchmark.html")
def benchmark():
    # data needed: health_score_summary, usage_summary, metadata (floor area, descriptions, categories)
    return render_template('benchmark.html', devices = api_bp.devices().json, masterlist = api_bp.usageoffline())
...
def health_check():
    ...
    if len(hc_latest) > 0:
        return render_template('health-check.html', devices = api_bp.devices().json, masterlist = api_bp.usageoffline(), hc_latest = hc_latest, hc_meta = api_bp.hc_meta(), context = context.view_all())
    else:
        return render_template('health-check.html', devices = api_bp.devices().json, masterlist = api_bp.usageoffline(), hc_latest = api_bp.devices().json, hc_meta = api_bp.hc_meta(), context = context.view_all())

@dashboard_bp.route("/anomaly.html")
#@required_user_level(3)
def anomaly():
    return render_template('anomaly.html', devices = api_bp.devices().json, masterlist = api_bp.usageoffline())

@dashboard_bp.route("/energy-usage.html")
#@required_user_level(3)
def energyusage():
    return render_template('energy-usage.html', devices = api_bp.devices().json, masterlist = api_bp.usageoffline())

@dashboard_bp.route("/context.html")
@required_user_level("USER_LEVEL_VIEW_COMMENTS")
def context_view():
    data = context.view_all()
    return render_template('context.html', data = data, devices = api_bp.devices().json, masterlist = api_bp.usageoffline())
...
#def start():
#    return render_template('start.html', devices = api_bp.devices().json, masterlist = api_bp.usageoffline())
...
#@required_user_level(3)
#def devices():
#    return render_template('devices.html', devices = api_bp.devices().json, masterlist = api_bp.usageoffline())
